...
function expectedMsgDataLength(
  ...
  function generate(bytes32 a, bytes32 b, int192[] memory c) public {
  ...
  function decodeReport(bytes calldata _report) public view returns (bytes32 a, bytes32 b, int192[] memory c){
  ...
  function verifySign(bytes calldata _report, bytes32 _rs, bytes32 _ss, uint8 _rawVs) public {
  ...
  function verifySign2(bytes calldata _report, bytes32 _rs, bytes32 _ss, uint8 _rawVs) public {
    reportHash = keccak256(_report);
    publicKey = ecrecover(reportHash, _rawVs, _rs, _ss);
  }

  function verifySign3(bytes calldata _report, bytes32 _rs, bytes32 _ss, uint8 _rawVs) public {
    reportHash = keccak256(_report);
    reportHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", reportHash));
    publicKey = ecrecover(reportHash, _rawVs, _rs, _ss);
  }
